...
------------------------------------------------------------------
---- ROSÉ PINE RICE — autostart + extras (added by rice setup) ----
------------------------------------------------------------------
...
-- Screenshot: whole screen (SUPER + PRTSC) / region (SUPER + SHIFT + S)
hl.bind(mainMod .. " + PRINT", hl.dsp.exec_cmd("grim $(xdg-user-dir PICTURES)/screenshot-$(date +%Y%m%d-%H%M%S).png"))
hl.bind(mainMod .. " + SHIFT + S", hl.dsp.exec_cmd("grim -g \"$(slurp)\" $(xdg-user-dir PICTURES)/screenshot-$(date +%Y%m%d-%H%M%S).png"))

-- Clipboard history (SUPER + V via cliphist + wofi)
hl.bind(mainMod .. " + SHIFT + V", hl.dsp.exec_cmd("cliphist list | wofi --dmenu -p  clipboard | cliphist decode | wl-paste"))

-- Launcher is already SUPER + R (hyprlauncher) — rebind to wofi for the rice
...
------------------------------------------------------------------
---- MATUGEN: apply wallpaper-derived border colors (if present) ----
---- Generated by matugen on every wallpaper change; reloaded live. ----
------------------------------------------------------------------
pcall(dofile, (os.getenv("HOME") or "") .. "/.config/hypr/matugen-colors.lua")
-- Re-theme from whatever wallpaper is CURRENTLY displayed (covers manual swaps,
-- variety clock/quote effects, etc. that matugen wouldn't otherwise see)
hl.bind(mainMod .. " + SHIFT + X", hl.dsp.exec_cmd("retheme-current.sh &"))
